...
import { draftManager } from "../ai/draft-manager.js";
...
export const draftToolDefinitions: Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      "IMPORTANT: Only call this AFTER the user has explicitly said they approve. " +
  ...
  },
  {
    ...
    description:
      ...
      "You MUST have shown the draft review summary to the user and received explicit approval before calling this. " +
      "Creates issues in JIRA in the correct order (Epics first, then Stories, then Tasks/Bugs, then Sub-tasks). " +
      "Use dry_run=true first to simulate and show the user what will be created, then call again with dry_run=false after confirmation.",
    inputSchema: {
      type: "object",
      properties: {
        draft_id: { type: "string" },
  ...
  switch (toolName) {
    case "create_jira_draft": {
      ...
      return (
        review +
        "\n\n" +
        "⏳ **ACTION REQUIRED:** Present this review summary to the user and wait for their decision.\n" +
        "Do NOT call approve_jira_draft or commit_jira_draft until the user explicitly approves."
      );
    }
    ...
    case "commit_jira_draft": {
      ...
      if (dryRun) {
        return (
          result +
          "\n\n⏳ **Present this dry-run report to the user.** If they confirm, call commit_jira_draft again with dry_run=false."
        );
      }
      return result;
    }
...
async function commitDraft(
  ...
  for (const artifact of sorted) {
    try {
      ...
      // Set story points via update — the field is often not on the Create screen
      if (artifact.storyPoints !== undefined && artifact.storyPoints !== null) {
        try {
          ...
          /* non-fatal — story points field may not be on the Edit screen either */
        }
      }
      ...
      // Set sprint via update — the field is often not on the Create screen
      if (artifact.sprintId !== undefined && artifact.sprintId !== null) {
        try {
          await jiraClient.updateIssue(issue.key, {
            [Config.jira.fields.sprint]: artifact.sprintId,
          });
        } catch {
          /* non-fatal — sprint field may not be on the Edit screen */
        }
      }
    } catch (err: unknown) {
...
function buildIssueFields(
  ...
  const issueFields: JiraIssueFields = {
    summary: artifact.summary.slice(0, 255),
    issuetype: { name: artifact.type === "Spike" ? "Story" : artifact.type },
    project: { key: projectKey },
    description: buildDescription(artifact),
  };

  // Only include priority if set (avoids sending invalid values)
  if (artifact.priority) {
    issueFields.priority = { name: artifact.priority as IssuePriority };
  }

  // Labels — always include, with ai-generated tag
  issueFields.labels = [
    ...(artifact.labels ?? []),
  ...
  // Story points are set via a separate updateIssue call after creation
  // to avoid 400 errors when the field is not on the Create Issue screen.

  // Components — only if non-empty
  ...
  // Sprint — set via separate update to avoid 400 on Create screen
  // (handled in commitDraft after creation)

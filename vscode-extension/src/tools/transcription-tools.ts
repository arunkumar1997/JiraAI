    ...
    inputSchema: {
      ...
      properties: {
        audio_file_path: {
          type: "string",
          description:
            "Absolute path to the recording (.mp4, .mp3, .wav, .m4a, .webm, .mkv, .mov, .ogg, .flac).",
        },
        model: {
          type: "string",
          enum: [...ALLOWED_MODELS],
          default: "base.en",
          description:
            "Whisper model. tiny/base = fastest; small/medium = more accurate.",
        },
        language: {
          type: "string",
          default: "auto",
          description:
            "ISO 639-1 language code (e.g. 'en') or 'auto' for auto-detect.",
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: {
          ...
          default: "base.en",
          description:
            "Whisper model. base.en is recommended for English meetings (fastest + accurate).",
        },
        language: {
          type: "string",
          default: "auto",
...
function validateInputs(
  ...
  if (language !== "auto" && !LANGUAGE_RE.test(language))
    throw new Error(
      `Invalid language '${language}'. Use ISO 639-1 like 'en', 'es', or 'auto'.`,
    );
...
async function transcribeMeeting(
  ...
  const filePath = validateInputs(
    args.audio_file_path as string | undefined,
    model,
    language,
  );

  const { transcript, engine, detectedLanguage } = await runWhisperSync(
    filePath,
    model,
    language,
  );
  ...
  return new Promise((res, rej) => {
    ...
    child.on("close", (code) => {
      try {
        ...
        if (!parsed.ok)
          rej(
            new Error(
              `Whisper error: ${parsed.error as string}\n\nRun: pip install faster-whisper`,
            ),
          );
        else
          res({
    ...
    child.on("error", (err: Error) => {
      if ((err as NodeJS.ErrnoException).code === "ENOENT") {
        rej(
          new Error(
            "python3 not found in PATH. Install Python 3 then: pip install faster-whisper",
          ),
        );
      } else {
        rej(err);
